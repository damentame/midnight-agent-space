...
class ClaudeCliRunner:
    def __init__(self) -> None:
        self._capability_cache: Dict[str, ClaudeCliCapabilities] = {}
    ...
    async def detect_capabilities(self, cli_binary: str) -> ClaudeCliCapabilities:
        ...
        capabilities = ClaudeCliCapabilities(
            supports_print="-p" in root_help or "--print" in root_help,
            supports_stream_json="stream-json" in root_help,
            supports_verbose="--verbose" in root_help,
            supports_model="--model" in root_help,
            supports_permission_mode="--permission-mode" in root_help,
            supports_allowed_tools="--allowedtools" in root_help.replace("_", ""),
            supports_add_dir="--add-dir" in root_help,
        )
        self._capability_cache[resolved] = capabilities
        return capabilities

    @staticmethod
    def _sanitize_permission_mode(mode: Optional[str]) -> str:
        value = (mode or "").strip().lower()
        if not value:
            return "acceptEdits"
        if value in {"bypass", "bypasspermissions", "danger", "dangerously-skip-permissions"}:
            return "acceptEdits"
        return mode.strip() if mode else "acceptEdits"

    def build_command(
        self,
        cli_binary: str,
        plan: ClaudeCliRunPlan,
        capabilities: ClaudeCliCapabilities,
        *,
        include_prompt: bool = True,
    ) -> List[str]:
        # Claude requires the prompt argument immediately after -p (not after other flags).
        cmd: List[str] = [cli_binary]
        if include_prompt and plan.prompt:
            cmd.extend(["-p", plan.prompt])
        else:
            cmd.append("-p")
        if plan.output_json and capabilities.supports_stream_json:
            # Claude CLI requires --verbose when using -p with stream-json.
        ...
        if capabilities.supports_permission_mode:
            cmd.extend(["--permission-mode", self._sanitize_permission_mode(plan.permission_mode)])
        if plan.allowed_tools and capabilities.supports_allowed_tools:
            cmd.extend(["--allowedTools", plan.allowed_tools])
        if plan.worktree_path and capabilities.supports_add_dir:
            cmd.extend(["--add-dir", plan.worktree_path])
        if plan.extra_args:
    ...
    def _prompt_file_path(worktree_path: Optional[str]) -> Optional[Path]:
        if not worktree_path:
            return None
        target = Path(worktree_path) / ".midnight" / "claude-prompt.md"
        try:
    ...
    def _execute_sync_subprocess(
        ...
        *,
        ...
        try:
            ...
            return {
                "ok": False,
                "error": f"cli binary not found: {command[0]}",
                "command": command,
            ...
            return {
                "ok": False,
                "error": f"failed to launch Claude CLI: {type(exc).__name__}: {exc}",
                "command": command,
                "capabilities": asdict(capabilities),
                "event_count": 0,
                "stdout_tail": [],
        ...
        emit(
            {
                ...
                "event_order": None,
                "payload": {"exit_code": exit_code, "timed_out": timed_out, "error": wait_error},
            }
        )
    ...
    async def _execute_sync_subprocess_async(
        ...
        *,
        ...
        """Run sync subprocess without blocking the event loop (avoids deadlock on Windows)."""
    ...
    async def execute_streaming(
        ...
        *,
        ...
        effective_prompt, prompt_file, prompt_delivery = self._resolve_prompt_delivery(plan, include_prompt=True)
        command_plan = ClaudeCliRunPlan(
